...
resourcegroup_name = 'rch'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'rch'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
print('Created entry for Chevrolet Impala...')
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carsmenu'", select='make,cost')
for item in items:
